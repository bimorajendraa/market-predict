════════════════════════════════════════════════════════════
  INVESTMENT MEMO: BMRI.JK
  Period: Q4-2025
  Date: 2026-02-21 20:17 WIB
════════════════════════════════════════════════════════════

Rating: Hold/Watch | Confidence: 25% | Integrity: N/A
────────────────────────────────────────────────────────────

1. COMPANY SNAPSHOT
────────────────────────────────────────
  Company: PT Bank Mandiri (Persero) Tbk
  Sector: Financial Services — Banks - Regional
  Country: Indonesia
  Employees: 38,751
  Market Cap: $478.3T
  Website: https://bankmandiri.co.id

  PT Bank Mandiri (Persero) Tbk provides various banking products and services to individuals and businesses in Indonesia, Singapore, Malaysia, Hong Kong, Timor Leste, Shanghai, England, and the Cayman Islands. It offers savings and current accounts, multicurrency, payroll, NOW, and foreign currency s...

2. INVESTMENT THESIS
────────────────────────────────────────
  Status: INSUFFICIENT DATA

  BASE: Quality banking company with sustainable competitive advantages.
  BULL: Strong NIM expansion with controlled credit costs and digital banking gains.
  BEAR: Rising NPLs, NIM compression from rate cuts, fintech competition eroding deposits.

  UPCOMING CATALYSTS:
    • Earnings Release — 2026-04-29
    • Ex-Dividend Date — 2026-01-06

3. VARIANT VIEW
────────────────────────────────────────
  Current Price: $5125.00

  BASE CASE (60% probability):
    Revenue growth: 12.8% (continues current trajectory)
    Op margin: 58.8% (stable)
    Price target: $5637.50 (+10%)
    Thesis: Quality banking company with sustainable competitive advantages.

  BULL CASE (25% probability):
    Revenue growth: 19.2% (accelerates)
    Op margin: 61.8% (+300bp expansion)
    Price target: $6662.50 (+30%)
    Thesis: Strong NIM expansion with controlled credit costs and digital banking gains.

  BEAR CASE (15% probability):
    Revenue growth: 3.8% (decelerates materially)
    Op margin: 53.8% (-500bp compression)
    Price target: $3843.75 (-25%)
    Thesis: Rising NPLs, NIM compression from rate cuts, fintech competition eroding deposits.

  PROBABILITY-WEIGHTED TARGET: $5624.69 (+9.8%)

4. KEY QUESTIONS
────────────────────────────────────────
  If any answer changes, revisit the rating.

  Q1. Is Net Interest Margin (NIM) expanding or compressing?
      Trigger: If NIM declines > 20bp QoQ for 2 consecutive quarters, downgrade.

  Q2. Are non-performing loans (NPLs) trending up?
      Trigger: If NPL ratio > 3% or rising > 50bp QoQ, flag credit risk.

  Q3. Is capital adequacy sufficient for growth + stress scenarios?
      Trigger: If CAR < 12% or declining toward regulatory minimum, critical risk.

  Q4. Is the cost-to-income ratio improving with digital adoption?
      Trigger: If CIR > 50% and not improving, efficiency concerns.

  Q5. Is loan growth outpacing deposit growth (funding gap)?
      Trigger: If LDR > 95%, liquidity risk increases; < 70% suggests under-deployment.


5. FINANCIAL QUALITY
────────────────────────────────────────
  Composite Score: 58.6/100 (sector: banking)

  Factor                Score   Coverage
  ──────────────────── ────── ──────────
  Quality               60.9        5/5
  Growth                78.7        3/4
  Balance Sheet          0.0        0/4
  Cashflow              36.8        2/4
  Shareholder           48.5        4/4

  KEY METRICS:
    ▲ Operating Margin: 58.77% (score: 100)
    ▲ Net Margin: 38.87% (score: 100)
    ─ Roe: 19.14% (score: 58)
    ▼ Roic: 2.33% (score: 7)
    ▼ Gross Margin: 0.00% (score: 0)
    ─ Revenue Growth: 12.80% (score: 52)
    ▲ Earnings Growth: 34.90% (score: 90)
    ▲ Eps Growth: 34.90% (score: 100)
    ▼ Cfo To Net Income: -348.28% (score: 0)
    ▲ Capex Intensity: 5.18% (score: 83)
    ▲ Dividend Yield: 30.00% (score: 100)
    ▼ Payout Ratio: 93.86% (score: 0)
    ─ Buyback Yield: 0.08% (score: 31)
    ─ Dilution: 0.00% (score: 53)

  ⚠ COVERAGE CONTRACT FAILED
    Missing: capital_adequacy_ratio, cost_to_income, net_interest_margin, non_performing_loan
    Rating locked: Coverage contract failed: 1/3 required metrics. Rating locked to Hold.

6. MOAT & EXECUTION
────────────────────────────────────────
  Competitive Position: MODERATE
  Adequate returns but limited pricing power visibility.

  Growth Trajectory: ACCELERATING — strong momentum across metrics

  Cash Conversion: POOR — weak cash generation requires monitoring

7. RISKS
────────────────────────────────────────
  IDIOSYNCRATIC RISKS: No significant flags triggered.

  MACRO RISKS:
    • Interest rate direction impacting NIM and bond portfolios
    • Credit cycle deterioration leading to rising provisions
    • Regulatory capital requirement changes

8. VALUATION
────────────────────────────────────────
  Verdict: FAIR

  CURRENT MULTIPLES:
    PE/TRAILING: 8.5x
    PE/FORWARD: 7.7x
    PB/RATIO: 1.6x
    EV/REVENUE: 4.2x
    DIVIDEND/YIELD: 1.9x

  3-STATEMENT MODEL (current: Rev $144817.49B, OpM 58.8%, NetM 38.9%):
  Year      Revenue   OpIncome   OpM%  NetIncome        FCF  Growth
  ────── ────────── ────────── ────── ────────── ────────── ───────
  Y1     $163354.1B $ 95996.7B  58.8% $ 75837.4B $ 80738.0B  12.8%
  Y2     $182662.6B $ 98773.9B  54.1% $ 78031.4B $ 83511.2B  11.8%
  Y3     $202463.2B $ 99982.4B  49.4% $ 78986.1B $ 85060.0B  10.8%
  Y4     $222426.1B $ 99405.6B  44.7% $ 78530.4B $ 85203.2B   9.9%
  Y5     $242177.5B $ 96871.0B  40.0% $ 76528.1B $ 83793.4B   8.9%
  Assumptions: growth=12.8%, tax=21%, target_margin=40.0%


  SENSITIVITY TABLE (current: $5125.0, base FCF/sh: $155.16):
  WACC       g=1.0%     g=2.0%     g=2.5%     g=3.0%     g=4.0%
  ────── ────────── ────────── ────────── ────────── ──────────
  8%     $ 3668.08 $ 4163.97 $ 4479.54 $ 4858.22 $ 5899.59
  9%     $ 3185.85 $ 3544.31 $ 3764.89 $ 4022.25 $ 4691.36
  10%    $ 2811.55 $ 3080.34 $ 3241.62 $ 3425.94 $ 3886.74
  11%    $ 2512.76 $ 2720.15 $ 2842.15 $ 2979.40 $ 3312.71
  12%    $ 2268.87 $ 2432.58 $ 2527.35 $ 2632.66 $ 2882.77

  HISTORICAL CONTEXT (5Y):
    price_percentiles: current=N/A, median=4363.43

9. CATALYST CALENDAR
────────────────────────────────────────
  Date                 Event                               Impact  
  ──────────────────── ─────────────────────────────────── ────────
  2026-04-29           Earnings Release                    HIGH    
  2026-01-06           Ex-Dividend Date                    MEDIUM  

  NOTE: Verify dates—corporate calendars update after each earnings release.

10. POSITIONING
────────────────────────────────────────
  INVESTMENT STYLE:
    VALUE: Below-median multiples, stable cash flows, dividend focus
    QUALITY: High margins, strong ROIC, consistent execution

  POSITION SIZING:
    Tier: EXPLORATORY
    Suggested weight: 0.5-2% of portfolio
    Rationale: Thesis at-risk OR coverage gaps OR turnaround play

  SUITABLE FOR:
    ✓ Value/income portfolios
    ✓ Quality compounders portfolios

11. TRACKING DASHBOARD
────────────────────────────────────────
  Thesis Status: INSUFFICIENT_DATA

  KPIs TO MONITOR:
    • Net Interest Margin: — (target: > 3%)
    • Non-Performing Loans: — (target: < 3%)
    • Capital Adequacy Ratio: — (target: > 12%)
    • Cost-to-Income ratio: — (target: < 45%)
    • Loan growth (YoY): — (target: 8-15%)

  WHAT WOULD CHANGE OUR MIND:
    [CRITICAL] If NPL ratio > 5% → Downgrade to SELL
    [CRITICAL] If CAR below regulatory minimum → Immediately reassess
    [WARNING] If NIM compression > 50bps in single quarter → Monitor closely
    [WARNING] If cost_to_income > 55% → Review efficiency

  DATA GAPS:
    Required: capital_adequacy_ratio, cost_to_income, net_interest_margin, non_performing_loan
    Recommended: casa_ratio, deposit_growth, loan_growth, loan_to_deposit_ratio, roa

12. EVIDENCE APPENDIX
────────────────────────────────────────
  Sources (6): Tier A: 0 | Tier B: 3 | Tier C: 3
    [Tier B][market_data] https://finance.yahoo.com/quote/BMRI.JK
    [Tier B][financial_data] https://finance.yahoo.com/quote/BMRI.JK/financials
    [Tier B][valuation] https://finance.yahoo.com/quote/BMRI.JK/analysis
    [Tier C][database_price_history] db://market_prices/BMRI.JK
    [Tier C][database_financial_facts] db://financial_facts/BMRI.JK
    [Tier C][news_catalysts] db://news_sentiment/BMRI.JK

  AUDIT TRAIL:
    Run ID: c7769bab-9297-4153-b8a5-4554ca0cc3f7
    Sources used: 0

  Generated: 2026-02-21 20:17:39 WIB
  Disclaimer: This memo is auto-generated. Verify all data before investment decisions.
════════════════════════════════════════════════════════════