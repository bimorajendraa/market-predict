════════════════════════════════════════════════════════════
  INVESTMENT MEMO: ORCL
  Period: Q4-2025
  Date: 2026-02-19 11:32 WIB
════════════════════════════════════════════════════════════

Rating: Buy | Confidence: 75% | Integrity: N/A
────────────────────────────────────────────────────────────

1. COMPANY SNAPSHOT
────────────────────────────────────────
  Company: Oracle Corporation
  Sector: Technology — Software - Infrastructure
  Country: United States
  Employees: 162,000
  Market Cap: $448.8B
  Website: https://www.oracle.com

  Oracle Corporation offers products and services that address enterprise information technology environments worldwide. Its Oracle cloud software as a service offering include various cloud software applications, including Oracle Fusion cloud enterprise resource planning ERP, Oracle Fusion cloud ente...

2. INVESTMENT THESIS
────────────────────────────────────────
  Status: ON TRACK

  BASE: Quality tech company with sustainable competitive advantages.
  BULL: Sustained cloud transition driving double-digit revenue growth with margin expansion.
  BEAR: Cloud growth stalls, legacy business declines faster than expected, margin compression.

  UPCOMING CATALYSTS:
    • Earnings Release — 2026-03-10
    • Ex-Dividend Date — 2026-01-09

3. VARIANT VIEW
────────────────────────────────────────
  Current Price: $156.17

  BASE CASE (60% probability):
    Revenue growth: 14.2% (continues current trajectory)
    Op margin: 32.0% (stable)
    Price target: $171.79 (+10%)
    Thesis: Quality tech company with sustainable competitive advantages.

  BULL CASE (25% probability):
    Revenue growth: 21.3% (accelerates)
    Op margin: 35.0% (+300bp expansion)
    Price target: $203.02 (+30%)
    Thesis: Sustained cloud transition driving double-digit revenue growth with margin expansion.

  BEAR CASE (15% probability):
    Revenue growth: 4.3% (decelerates materially)
    Op margin: 27.0% (-500bp compression)
    Price target: $117.13 (-25%)
    Thesis: Cloud growth stalls, legacy business declines faster than expected, margin compression.

  PROBABILITY-WEIGHTED TARGET: $171.40 (+9.7%)

4. KEY QUESTIONS
────────────────────────────────────────
  If any answer changes, revisit the rating.

  Q1. Is the cloud/SaaS transition accelerating or decelerating?
      Trigger: If recurring revenue mix < 50% or cloud growth < 15%, downgrade.

  Q2. Is stock-based compensation (SBC) diluting shareholder value?
      Trigger: If SBC > 10% of revenue or dilution > 3% per year, flag risk.

  Q3. Is R&D spend generating returns or being cut?
      Trigger: If R&D < 10% of revenue (for software), competitive moat weakens.

  Q4. Is free cash flow conversion healthy despite capex investments?
      Trigger: If FCF margin < 10% for 2+ quarters while capex rises, flag cash burn.

  Q5. Is the total addressable market (TAM) expanding or saturating?
      Trigger: If revenue growth decelerating 3+ consecutive quarters, potential TAM issue.


5. FINANCIAL QUALITY
────────────────────────────────────────
  Composite Score: 57.1/100 (sector: tech)

  Factor                Score   Coverage
  ──────────────────── ────── ──────────
  Quality               75.0        5/5
  Growth                83.4        3/4
  Balance Sheet         21.1        3/4
  Cashflow              18.5        4/4
  Shareholder           65.9        4/4

  KEY METRICS:
    ▲ Operating Margin: 32.00% (score: 78)
    ▲ Net Margin: 25.28% (score: 84)
    ▲ Roe: 69.03% (score: 100)
    ▼ Roic: 6.86% (score: 21)
    ▲ Gross Margin: 68.54% (score: 98)
    ─ Revenue Growth: 14.20% (score: 56)
    ▲ Earnings Growth: 90.90% (score: 100)
    ▲ Eps Growth: 94.70% (score: 100)
    ▼ Debt To Equity: 432.51% (score: 0)
    ▼ Current Ratio: 90.90% (score: 23)
    ─ Net Debt To Ebitda: 426.27% (score: 51)
    ▼ Fcf Margin: -16.73% (score: 0)
    ─ Cfo To Net Income: 144.54% (score: 68)
    ▼ Capex Intensity: 34.77% (score: 8)
    ▼ Fcf Yield: -2.27% (score: 0)
    ▲ Dividend Yield: 128.00% (score: 100)
    ▲ Payout Ratio: 35.71% (score: 78)
    ─ Buyback Yield: 0.33% (score: 35)
    ─ Dilution: 1.89% (score: 38)

6. MOAT & EXECUTION
────────────────────────────────────────
  Competitive Position: STRONG
  High margins and returns suggest pricing power and durable advantages.

  Growth Trajectory: ACCELERATING — strong momentum across metrics

  Cash Conversion: POOR — weak cash generation requires monitoring

7. RISKS
────────────────────────────────────────
  IDIOSYNCRATIC RISKS:
    [WARNING] High leverage: D/E ratio above 2.0x

  MACRO RISKS:
    • Rising interest rates reducing growth stock multiples
    • Regulatory pressure (antitrust, data privacy, AI governance)
    • Geopolitical tensions affecting supply chains and market access

8. VALUATION
────────────────────────────────────────
  Verdict: FAIR

  CURRENT MULTIPLES:
    PE/TRAILING: 29.3x
    PE/FORWARD: 19.8x
    PB/RATIO: 15.0x
    EV/EBITDA: 21.4x
    EV/REVENUE: 9.2x
    DIVIDEND/YIELD: 1.3x
    FCF/YIELD: -0.0x

  3-STATEMENT MODEL (current: Rev $61.02B, OpM 32.0%, NetM 25.3%):
  Year      Revenue   OpIncome   OpM%  NetIncome        FCF  Growth
  ────── ────────── ────────── ────── ────────── ────────── ───────
  Y1     $    69.7B $    22.3B  32.0% $    17.6B $    19.7B  14.2%
  Y2     $    78.8B $    25.4B  32.2% $    20.1B $    22.4B  13.1%
  Y3     $    88.2B $    28.7B  32.5% $    22.6B $    25.3B  12.0%
  Y4     $    97.8B $    32.0B  32.7% $    25.3B $    28.2B  10.8%
  Y5     $   107.3B $    35.4B  33.0% $    28.0B $    31.2B   9.7%
  Assumptions: growth=14.2%, tax=21%, target_margin=33.0%

  DCF: Negative FCF — DCF not meaningful. Use revenue multiples instead.
    Current Price: $156.17, FCF/share: $-3.55

  SENSITIVITY TABLE (current: $156.17, base FCF/sh: $7.76):
  WACC       g=1.0%     g=2.0%     g=2.5%     g=3.0%     g=4.0%
  ────── ────────── ────────── ────────── ────────── ──────────
  8%     $  193.97 $  220.34 $  237.12 $  257.26 $  312.63
  9%     $  168.34 $  187.41 $  199.14 $  212.82 $  248.40
  10%    $  148.46 $  162.75 $  171.33 $  181.13 $  205.64
  11%    $  132.59 $  143.62 $  150.11 $  157.40 $  175.13
  12%    $  119.64 $  128.34 $  133.38 $  138.98 $  152.28

  HISTORICAL CONTEXT (5Y):
    price_percentiles: current=N/A, median=105.93

9. CATALYST CALENDAR
────────────────────────────────────────
  Date                 Event                               Impact  
  ──────────────────── ─────────────────────────────────── ────────
  2026-03-10           Earnings Release                    HIGH    
  2026-01-09           Ex-Dividend Date                    MEDIUM  
  2026-01-23           Dividend Payment                    LOW     

  NOTE: Verify dates—corporate calendars update after each earnings release.

10. POSITIONING
────────────────────────────────────────
  INVESTMENT STYLE:
    QUALITY: High margins, strong ROIC, consistent execution

  POSITION SIZING:
    Tier: STANDARD
    Suggested weight: 2-4% of portfolio
    Rationale: Composite score > 50, no critical risk flags

  SUITABLE FOR:
    ✓ Quality compounders portfolios

11. TRACKING DASHBOARD
────────────────────────────────────────
  Thesis Status: ON_TRACK

  KPIs TO MONITOR:
    • Cloud revenue growth (%): — (target: > 20%)
    • Operating margin expansion: 32.00% (target: expanding QoQ)
    • Free cash flow margin: -16.73% (target: > 15%)
    • R&D as % of revenue: — (target: 10-20%)
    • Revenue growth (YoY): 14.20% (target: > 8%)

  WHAT WOULD CHANGE OUR MIND:
    [CRITICAL] If revenue_growth < 0 for 2 consecutive quarters → Downgrade to SELL
    [CRITICAL] If fcf turns negative for 2 consecutive quarters → Downgrade thesis
    [WARNING] If operating_margin declines > 500bps YoY → Review thesis
    [CRITICAL] If major customer loss (>10% revenue) → Reassess moat

  DATA GAPS:
    Recommended: revenue_qoq, share_count

12. EVIDENCE APPENDIX
────────────────────────────────────────
  No evidence items collected.

  AUDIT TRAIL:
    Run ID: 0acf1f42-db56-4490-9a2b-c353d63924ce
    Sources used: 0

  Generated: 2026-02-19 11:32:03 WIB
  Disclaimer: This memo is auto-generated. Verify all data before investment decisions.
════════════════════════════════════════════════════════════